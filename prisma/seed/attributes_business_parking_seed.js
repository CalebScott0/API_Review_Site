...
async function processCSV() {
  ...
  const parser = fs
    .createReadStream(
      '/Users/cbs062/Desktop/Review_Site_CSV_Files/business_parking.csv'
    )
  ...
  for await (const record of parser) {
    if (record[1].length) {
      ...
      // array of json values for parking types for 1 business
      const parking_type_array = record[1]
        .slice(1, record[1].length - 1)
        .split(', ');

      // slice out type and value for db records
      parking_type_array.forEach((val) => {
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    try {
      await prisma.attribute_business_parking.createMany({
        data: create_batch,
        skipDuplicates: true,
      });
  ...
  console.log('\nparking type attribute records seeded');
}
